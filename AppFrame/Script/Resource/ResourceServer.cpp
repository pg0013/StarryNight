...
void ResourceServer::RegisterModel(const std::string _mode, const std::string _filename, bool _async)
{
	auto iter = map_model_.find(_filename);
	...
	//マップにキーとデータを登録
	map_model_.emplace(_filename, tmp);
}

int ResourceServer::GetModelHandle(const std::string _filename)
{
	auto iter = map_model_.find(_filename.c_str());
	if (iter == map_model_.end())
	{
